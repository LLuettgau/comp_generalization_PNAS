function [prior_sched, prior_sched_prim, prior_sched_sec, trans_sched, trans_sched_prim, trans_sched_sec, all_exp_stim_names, all_trans_stim_names, dims_SF, dims_SR, ...
          ...
          preprocess_data_recovery(task_data, subj, feature_names, model_type)
    ...
    if model_type == 1 || model_type == 3
        ...
        [exp_probes, exp_options, ...
            exp_num_faulty_trials] = get_probes_recovery(experimental_data, all_stim_names, probe_type);

        %% inference probes (prior learning)
        %find inference probe trials
        probe_type = 'inferenceTrial';
        [inf_probes, inf_options, ...
            inf_num_faulty_trials] = get_probes_recovery(experimental_data, all_stim_names, probe_type);
        ...
        [trans_exp_probes, trans_exp_options, ...
            trans_exp_num_faulty_trials] = get_probes_recovery(experimental_data, all_stim_names, probe_type);

        %% inference probes (transfer learning)
        %find transfer inference probe trials
        probe_type = 'transferInferenceTrial';
        [trans_inf_probes, trans_inf_options, ...
            trans_inf_num_faulty_trials] = get_probes_recovery(experimental_data, all_stim_names, probe_type);
    ...
    if model_type >= 2
        ...
        probe_type = 'probeTrial';
        [exp_probes_features, exp_options_features, ...
           exp_num_faulty_trials_features] = get_probes_features_recovery(experimental_data, feature_names, probe_type);

        %% inference probes (prior learning)
        %find inference probe trials
        probe_type = 'inferenceTrial';
        [inf_probes_features, inf_options_features, ...
           inf_num_faulty_trials_features] = get_probes_features_recovery(experimental_data, feature_names, probe_type);

        %% experience probes (transfer learning)
        %find transfer exp probe trials
        probe_type = 'transferProbeTrial';
        [trans_exp_probes_features, trans_exp_options_features, ...
            trans_exp_num_faulty_trials_features] = get_probes_features_recovery(experimental_data, feature_names, probe_type);

        %% inference probes (transfer learning)
        %find transfer inference probe trials
        probe_type = 'transferInferenceTrial';
        [trans_inf_probes_features, trans_inf_options_features, ...
           trans_inf_num_faulty_trials_features] = get_probes_features_recovery(experimental_data, feature_names, probe_type);
